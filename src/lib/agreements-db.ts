...
export function agreementDataToRow(d: AgreementData, managerId: string | null) {
  ...
  return {
    ...
    parent_name: d.student.parentName,
    parent_phone: d.student.parentPhone,
    parent_email: d.student.parentEmail,
    parent_address: d.student.parentAddress,
    pg_name: d.pgName,
...
export function rowToAgreementData(r: DbAgreement): AgreementData {
  ...
  if (r.agreement_text_snapshot && r.payment_mode === "Instalments") {
    try {
  ...
  // Backward compat: old "Annual" rows get mapped to Instalments
  const paymentMode: AgreementData["paymentMode"] =
    r.payment_mode === "Monthly" ? "Monthly" : "Instalments";

  if (!instalments && r.payment_mode !== "Monthly" && r.agreement_text_snapshot) {
    try {
      const snap = JSON.parse(r.agreement_text_snapshot);
