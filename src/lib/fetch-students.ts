...
function parseStudentRow(row: Record<string, string>): Student {
  return {
    ...
    collegeName: getField(row, "College Name", "College", "Working At", "College Name / Working At"),
    studentId: getField(row, "College ID Number", "Student ID", "Student ID / Roll Number", "College ID", "Roll Number", "ID"),
