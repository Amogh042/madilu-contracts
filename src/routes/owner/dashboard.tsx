...
const initialAgreement = (s: Student): AgreementData => {
  ...
  const paymentMode: AgreementData["paymentMode"] =
    pmRaw.includes("annual") || pmRaw.includes("instalment") || pmRaw.includes("installment")
      ? "Instalments"
      : "Monthly";
  ...
  return {
    ...
    ownerAddress: "",
    residentAge: s.age || "",
    residentCollege: s.collegeName || "",
    residentStudentId: s.studentId || "",
    residentAadhaar: s.aadhaarNumber || "",
    parentFatherName: s.guardianName || "",
    parentAge: s.guardianAge || "",
    pgName: pg,
...
function OwnerDashboard() {
  ...
  const pendingAgreements = useMemo(
    () => agreements.filter((a) => a.status === "pending"),
    [agreements],
  );
  ...
  const handleAddManager = async () => {
    ...
    try {
      ...
      const display =
        msg.includes("duplicate key") || msg.includes("unique") || msg.includes("already exists")
  ...
  const generateDefaultTemplate = (): string => {
    const placeholder: AgreementData = {
      student: {
        name: "{RESIDENT_NAME}",
        dob: "",
        contactNumber: "{RESIDENT_PHONE}",
        email: "{RESIDENT_EMAIL}",
        permanentAddress: "{RESIDENT_ADDRESS}",
        age: "",
        collegeName: "",
        studentId: "",
        guardianName: "{PARENT_NAME}",
        guardianAddress: "{PARENT_ADDRESS}",
      ...
      ownerFatherName: "{OWNER_FATHER}",
      ownerAge: "{OWNER_AGE}",
      ownerAddress: "{OWNER_ADDRESS}",
      residentAge: "{RESIDENT_AGE}",
      residentCollege: "{COLLEGE}",
      residentStudentId: "{STUDENT_ID}",
      residentAadhaar: "",
      parentFatherName: "{PARENT_FATHER}",
      ...
      <header className="relative z-10 max-w-6xl mx-auto px-6 pt-10 pb-8 flex items-center justify-between">
        ...
        <div className="flex gap-2 items-center">
          <button
            ...
            className="glass glass-hover rounded-xl px-3 py-2 text-sm relative"
          >
            <svg
              className="w-5 h-5"
              fill="none"
          ...
          <button
            ...
            className="glass glass-hover rounded-xl px-4 py-2 text-sm text-red-300"
          >
            Logout
          </button>
        </div>
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-6 pb-20">
        {successMsg && (
          <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 text-emerald-300 p-3 text-sm mb-4 flex items-center justify-between">
            ...
            <button
              ...
              className="text-emerald-300/60 hover:text-emerald-300 ml-3 shrink-0"
            >
              &times;
            </button>
          </div>
        ...
        {errorMsg && (
          <div className="rounded-xl border border-red-500/30 bg-red-500/10 text-red-300 p-3 text-sm mb-4 flex items-center justify-between">
            ...
            <button
              onClick={() => setErrorMsg("")}
              className="text-red-300/60 hover:text-red-300 ml-3 shrink-0"
            >
              &times;
            </button>
          </div>
          ...
          <div className="space-y-8 animate-fade-in">
            <div className="grid sm:grid-cols-4 gap-5">
              ...
              <StatCard
                label="This Month"
                value={monthCount}
                hint={new Date().toLocaleDateString("en-IN", { month: "long", year: "numeric" })}
              />
              <StatCard
                label="Active Managers"
                value={managers.filter((m) => m.active).length}
                hint={`${managers.length} total`}
              />
            </div>
              ...
              <div className="glass rounded-3xl p-7 border border-yellow-500/20">
                ...
                <div className="space-y-2">
                  {pendingAgreements.slice(0, 5).map((a) => (
                    <div
                      key={a.id}
                      className="flex items-center justify-between gap-3 rounded-xl border border-white/5 bg-white/[0.02] p-4"
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-white/50 mt-0.5">
                          ...
                          <span className="text-[#4ECDC4]/70">by {getCreatorLabel(a)}</span>
                        </p>
                      </div>
                      <div className="flex gap-2 shrink-0">
                        <button
                          onClick={() => handleEdit(a)}
                          className="glass glass-hover rounded-lg px-3 py-2 text-xs"
                        >
                          View
                        </button>
                        <button
                          onClick={() => handleApprove(a.id)}
                          className="rounded-lg px-3 py-2 text-xs bg-green-500/20 text-green-300 hover:bg-green-500/30 transition"
                        >
                          Approve
                        </button>
                        <button
                          ...
                          className="rounded-lg px-3 py-2 text-xs bg-red-500/20 text-red-300 hover:bg-red-500/30 transition"
                        >
                          Reject
                        </button>
                      </div>
                  ...
                  {pendingAgreements.length > 5 && (
                    ...
                    >
                      View all {pendingAgreements.length} pending →
                    </button>
                  )}
                </div>
              </div>
            ...
            <div className="glass rounded-3xl p-10 text-center relative overflow-hidden">
              ...
              <div className="relative">
                <h2 className="font-display text-4xl sm:text-5xl font-bold">
                  Owner <span className="text-gold">Controls</span>
                </h2>
                <p className="text-white/60 mt-3 max-w-xl mx-auto">
                  Create agreements directly, manage your team, or edit the agreement template.
                </p>
                <div className="flex justify-center gap-3 mt-7 flex-wrap">
                  <button
                    onClick={startFlow}
                    className="btn-gold rounded-xl px-6 py-4 text-base font-semibold flex items-center gap-2"
                  >
                    <svg
                      className="w-5 h-5"
                      fill="none"
                  ...
                  <button
                    onClick={() => setView("extension-pick")}
                    className="glass glass-hover rounded-xl px-6 py-4 text-base font-medium flex items-center gap-2 border border-[#D4A853]/20"
                  >
                    <svg
                ...
                <div className="flex justify-center gap-3 mt-3 flex-wrap">
                  <button
                    ...
                    className="glass glass-hover rounded-xl px-6 py-4 text-base font-medium"
                  >
                    All Agreements
                  </button>
                  <button
        ...
        {view === "agreements" && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex gap-3 flex-wrap">
              <Select
                value={filterPg || "__all__"}
                onValueChange={(v) => setFilterPg(v === "__all__" ? "" : v)}
              >
                <SelectTrigger
                  className={
                    triggerCls +
                    " w-auto min-w-[160px] h-auto [&>svg]:text-[#D4A853] [&>svg]:opacity-100"
                  }
                >
                  <SelectValue />
                </SelectTrigger>
              ...
              >
                <SelectTrigger
                  className={
                    triggerCls +
                    " w-auto min-w-[140px] h-auto [&>svg]:text-[#D4A853] [&>svg]:opacity-100"
                  }
                >
                  <SelectValue />
                </SelectTrigger>
              ...
              <span className="self-center text-xs text-white/40">
                {filteredAgreements.length} results
              </span>
            </div>
              ...
              {filteredAgreements.length === 0 ? (
                ...
                <div className="space-y-2">
                  {filteredAgreements.map((a) => (
                    <div
                      key={a.id}
                      className="flex items-center justify-between gap-3 rounded-xl border border-white/5 bg-white/[0.02] p-4 hover:bg-white/[0.05] transition"
                    >
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <p className="font-medium truncate">{a.student_name}</p>
                          <span
                            className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full ${statusColors[a.status]}`}
                          >
                            {a.status}
                          </span>
                        </div>
                      ...
                      <div className="flex gap-2 shrink-0">
                        ...
                        <button
                          onClick={() => generateAgreementPDF(rowToAgreementData(a))}
                          className="glass glass-hover rounded-lg px-3 py-2 text-xs"
                        >
                          PDF
                        </button>
                        <button
                          ...
                          className="rounded-lg px-3 py-2 text-xs text-red-400 hover:bg-red-500/10 transition"
                        >
                          Del
                        </button>
                      </div>
          ...
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="glass rounded-3xl p-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mt-2">
                  {PG_LIST.map((p) => {
                    ...
                    return (
                      <label
                        key={p}
                        className={`flex items-center gap-2 cursor-pointer rounded-xl border px-3 py-2 text-sm transition-all ${checked ? "border-[#D4A853] bg-[#D4A853]/10" : "border-white/10 hover:border-white/20"}`}
                      >
                        <input
                          type="checkbox"
              ...
              <button
                ...
                disabled={
                  mgrAdding || !newMgr.name || !newMgr.phone || newMgr.pg_names.length === 0
                }
              ...
              {successMsg && (
                <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 text-emerald-300 p-3 text-sm">
                  {successMsg}
                </div>
              )}
            </div>
                ...
                <div className="space-y-2">
                  {managers.map((m) =>
                    editMgr?.id === m.id ? (
                      <div
                        key={m.id}
                        className="rounded-xl border border-[#D4A853]/30 bg-[#D4A853]/5 p-4 space-y-3"
                      >
                        <div className="grid sm:grid-cols-2 gap-3">
                          <input
                            ...
                            placeholder="Full Name *"
                            value={editMgr.name}
                            onChange={(e) => setEditMgr({ ...editMgr, name: e.target.value })}
                          />
                          <input
                            className={inputCls}
                            placeholder="Phone *"
                            value={editMgr.phone}
                            onChange={(e) => setEditMgr({ ...editMgr, phone: e.target.value })}
                          />
                          <input
                            className={inputCls}
                            placeholder="Email"
                            value={editMgr.email}
                            onChange={(e) => setEditMgr({ ...editMgr, email: e.target.value })}
                          />
                        </div>
                        <div>
                          <span className="text-xs uppercase tracking-wider text-white/50 font-medium">
                        ...
                        <div className="flex gap-2 justify-end">
                          <button
                            onClick={() => setEditMgr(null)}
                            className="glass glass-hover rounded-lg px-4 py-2 text-xs"
                          >
                            Cancel
                          </button>
                          <button
                            ...
                            disabled={
                              ...
                              !editMgr.name ||
                              !editMgr.phone ||
                              editMgr.pg_names.length === 0
                            }
                            className="btn-gold rounded-lg px-4 py-2 text-xs disabled:opacity-40"
                          >
                      ...
                      <div
                        key={m.id}
                        className={`flex items-center justify-between gap-3 rounded-xl border p-4 transition ${m.active ? "border-white/5 bg-white/[0.02]" : "border-red-500/10 bg-red-500/5 opacity-60"}`}
                      >
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            <p className="font-medium truncate">{m.name}</p>
                            {!m.active && (
                              <span className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full bg-red-500/15 text-red-300">
                                inactive
                              </span>
                            )}
                          </div>
                          ...
                          <div className="flex flex-wrap gap-1 mt-1">
                            {m.pg_name
                              ...
                              .map((pg) => (
                                <span
                                  key={pg}
                                  className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full bg-[#D4A853]/15 text-[#F5D799]"
                                >
                                  {pg}
                                </span>
                              ))}
                        ...
                        <div className="flex gap-2 shrink-0">
                          ...
                          <button
                            ...
                            className={`glass glass-hover rounded-lg px-3 py-2 text-xs ${m.active ? "text-yellow-300" : "text-green-300"}`}
                          >
                            {m.active ? "Deactivate" : "Activate"}
                          </button>
                          <button
                            onClick={() => setResetPwMgr({ id: m.id, name: m.name })}
                            className="glass glass-hover rounded-lg px-3 py-2 text-xs text-sky-300"
                          >
                            Reset Password
                          </button>
                          <button
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-white/50 text-sm mt-1">
                  Edit clause text. Dynamic fields are filled from form data.
                </p>
              </div>
              <button
                onClick={() => setView("home")}
            ...
            <div className="glass rounded-3xl p-6">
              ...
              <div className="flex justify-end mt-4 gap-3 items-center">
                {templateStatus === "saved" && (
                  <span className="text-[#4ECDC4] text-sm">Template saved successfully</span>
                )}
                {templateStatus && templateStatus !== "saved" && (
                  <span className="text-red-400 text-sm max-w-xs truncate">{templateStatus}</span>
                )}
                <button
              ...
              {notifications.length === 0 ? (
                ...
                <div className="space-y-2">
                  {notifications.map((n) => (
                    <div
                      key={n.id}
                      className={`rounded-xl border p-4 ${n.is_read ? "border-white/5 bg-white/[0.02]" : "border-[#D4A853]/20 bg-[#D4A853]/5"}`}
                    >
          ...
          <div>
            ...
            {step === 0 && (
              <StudentStep
                ...
                onNext={() => agreementData && setStep(1)}
              />
            )}
            {step === 1 && agreementData && (
              <DetailsStep
                data={agreementData}
        ...
        {view === "extension-pick" && (
          <AgreementStudentPicker
            ...
            onSelect={(a) => {
              setSecondaryDoc(a);
              setView("extension-preview");
            }}
          />
        )}
        {view === "extension-preview" && secondaryDoc && (
          <SecondaryDocPreview
            agreement={secondaryDoc}
            docType="extension"
            onBack={() => setView("extension-pick")}
          />
        )}
        ...
        {resetPwMgr && (
          <div
            ...
            onClick={() => setResetPwMgr(null)}
          >
            <div
              className="glass rounded-3xl p-8 max-w-md w-full mx-4 animate-fade-in"
              onClick={(e) => e.stopPropagation()}
            >
              <h3 className="font-display text-2xl font-bold mb-4">Reset Password</h3>
              <p className="text-sm text-white/70 mb-6">
                Reset password for{" "}
                <span className="text-[#F5D799] font-medium">{resetPwMgr.name}</span>? They will
                need to set a new password on their next login.
              </p>
              <div className="flex gap-3 justify-end">
                <button
                  onClick={() => setResetPwMgr(null)}
                  className="glass glass-hover rounded-xl px-5 py-2.5 text-sm"
                >
                  Cancel
                </button>
                <button
                  onClick={handleResetPassword}
                  className="rounded-xl px-5 py-2.5 text-sm bg-sky-500/20 text-sky-300 hover:bg-sky-500/30 transition font-semibold"
                >
                  Yes, Reset
                </button>
              </div>
            ...
            >
              ...
              <div className="flex gap-3 justify-end">
                ...
                <button
                  ...
                  className="rounded-xl px-5 py-2.5 text-sm bg-red-500/20 text-red-300 hover:bg-red-500/30 transition font-semibold"
                >
                  Reject
                </button>
              </div>
