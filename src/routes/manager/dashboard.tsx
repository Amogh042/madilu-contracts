...
function ManagerDashboard() {
  ...
  const [view, setView] = useState<
    "home" | "flow" | "history" | "notifications" | "extension-pick" | "extension-preview"
  >("home");
  ...
  const loadAgreements = useCallback(async () => {
    ...
    try {
      ...
      console.error("loadAgreements failed:", e);
    }
    setLoading(false);
  }, [session?.id]);
  ...
  useEffect(() => {
    loadAgreements();
    loadNotifications();
  }, [loadAgreements, loadNotifications]);
      ...
      <header className="relative z-10 max-w-6xl mx-auto px-6 pt-10 pb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-white/50 text-sm mt-1 tracking-wide">
            Manager — {session.name} · {session.pgNames.join(", ")}
          </p>
        </div>
            ...
            <div className="glass rounded-3xl p-10 text-center relative overflow-hidden">
              ...
              <div className="relative">
                <h2 className="font-display text-4xl sm:text-5xl font-bold">
                  Create a new <span className="text-gold">agreement</span>
                </h2>
                <p className="text-white/60 mt-3 max-w-xl mx-auto">
                  Agreements you create will be sent to the owner for approval.
                </p>
            ...
            <div className="glass rounded-3xl p-6">
              <div className="space-y-2">
                {agreements.map((a) => (
                  <div
                    key={a.id}
                    className="flex items-center justify-between gap-4 rounded-xl border border-white/5 bg-white/[0.02] p-4 hover:bg-white/[0.05] transition"
                  >
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <p className="font-medium truncate">{a.student_name}</p>
              ...
              {notifications.length === 0 ? (
                ...
                <div className="space-y-2">
                  {notifications.map((n) => (
                    <div
                      key={n.id}
                      className={`rounded-xl border p-4 transition ${n.is_read ? "border-white/5 bg-white/[0.02]" : "border-[#D4A853]/20 bg-[#D4A853]/5"}`}
                    >
