    ...
    <>
      {sections.map((sec, i) => {
        switch (sec.type) {
          case "header":
            return (
              <h3 key={i} className="text-center text-lg font-bold mt-4">
                {sec.text}
              </h3>
            );
          case "subheader":
            return (
              <p key={i} className="text-center text-sm mb-4">
                {sec.text}
              </p>
            );
          case "clause-title":
            return (
              <h4 key={i} className="font-bold mt-6 mb-2 text-[15px]">
                {sec.text}
              </h4>
            );
          case "section-title":
            return (
              <h5 key={i} className="font-bold mt-5 mb-2 text-[14px] uppercase tracking-wide">
                {sec.text}
              </h5>
            );
          case "paragraph":
            return (
              <p key={i} className="mb-3">
                {sec.segments ? <SegmentedText segments={sec.segments} /> : sec.text}
              </p>
            );
          case "sub-clause":
            return (
              <p key={i} className="mb-2" style={{ paddingLeft: (sec.indent || 0) * 16 }}>
                {sec.segments ? <SegmentedText segments={sec.segments} /> : sec.text}
              </p>
            );
          case "signature-row":
            ...
            return (
              <pre key={i} className="mt-4 mb-2 text-sm whitespace-pre-wrap font-serif">
                {sec.text}
              </pre>
            );
          case "witness-row":
          ...
          case "note":
            return (
              <p key={i} className="mt-6 text-xs italic text-gray-500 whitespace-pre-wrap">
                {sec.text}
              </p>
            );
          default:
    ...
    <div className="glass rounded-3xl p-8 animate-fade-in">
      <div className="mb-6">
        ...
        <p className="text-white/50 text-sm mt-1">
          {hidePdf
            ? "Review the agreement before submitting for approval"
            : "Review the full agreement before generating the stamp-paper PDF"}
        </p>
      </div>
      ...
      <div className="flex justify-between mt-8 gap-3 flex-wrap">
        ...
        <div className="flex gap-3">
          {!hidePdf && (
            <button
              onClick={() => generateAgreementPDF(data)}
          ...
          <button
            ...
            disabled={saving}
            className="btn-gold rounded-xl px-8 py-3 text-sm font-semibold disabled:opacity-40"
          >
